        ...
        if dominant_type != ExpressionType.NULL:
            if self.operador == '+':
                ...
                if dominant_type in [ExpressionType.NUMBER, ExpressionType.FLOAT]:
                    return Symbol(self.line, self.col, op1.value-op2.value, dominant_type)
                ast.set_errors(f'Operación incorrecta: "{op1.value} - {op2.value}".', self.line, self.col, 'Semántico')
                ...
                if op2.value == 0:
                    ast.set_errors(f'Operación incorrecta: "{op1.value} / 0".', self.line, self.col, 'Semántico')
                    return Symbol(self.line, self.col, None, ExpressionType.NULL)
            ...
            ast.set_errors(f'Operación incorrecta: "{self.operador}{op1.value}".', self.line, self.col, 'Semántico')
            return Symbol(self.line, self.col, None, ExpressionType.NULL)
 
        ast.set_errors(f'Operación incorrecta: "{op1.value} {self.operador} {op2.value}".', self.line, self.col, 'Semántico')
        return Symbol(self.line, self.col, None, ExpressionType.NULL)
